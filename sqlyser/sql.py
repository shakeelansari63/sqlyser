...
class SQL:
    ...
    FORMAT_NEWLINE_KEYWORD = (
        r"^SELECT |^SEL | SELECT | SEL | FROM | INNER\s+JOIN "
    ...
    def __str__(self):
        ...
        return (
            ...
            + "Encoder Dict: {enc_dict}".format(enc_dict=self.__get_encoder_dict())
        )

    def __get_encoder_dict(self) -> str:
        return "\n".join(
            [
                "{key} : {value}".format(key=key, value=value)
                for key, value in self._encoder_dict.items()
            ]
        )

    def __decode_sql(self, input_sql: str) -> str:
    ...
    def __is_valid_function(self, function_string: str):
        return not (" SELECT " in function_string or " JOIN " in function_string)

    def __encode_column_list(self) -> None:
        """Encode case and functions from main Clean SQL Query"""
        functions_list = re.compile(r"(?<=\s)[a-z0-9\_\.]+ \( .*? \)", re.I).findall(
        ...
        if len(functions_list) > 0:
            for index, func in enumerate(functions_list, start=1):
                if self.__is_valid_function(func):
                    ...
                    # Encode the Function
                    func_encoded_sql = self._clean_encoded_sql.replace(func, key)
                    self._clean_encoded_sql = func_encoded_sql

        # Find all column lists between SELECT and FROM Keywords
        select_from_list = re.compile(r"SELECT (.*?) FROM", re.I | re.DOTALL).findall(
            self._clean_encoded_sql
        )
        ...
        if len(select_from_list) > 0:
            for index, col_list_str in enumerate(select_from_list, start=1):
                ...
                # Encode
                encoded_col_sql = self._clean_encoded_sql.replace(col_list_str, key)
    ...
    def __clean_encode_sql(self):
        ...
        single_quoted_substr = single_quoted_string.findall(self._clean_encoded_sql)
        ...
        no_crlf_sql = (
            self._clean_encoded_sql.replace("\n", " ")
            .replace("\r", " ")
            .replace("\t", "")
        )
        ...
        paranthesis_sapce_sql = self._clean_encoded_sql.replace("(", " ( ").replace(
            ")", " ) "
        )
    ...
    def reformat(self, sql) -> str:
        # Initialize Reformatted SQL
        reformated_sql = sql

        new_line_keywords = re.compile(self.FORMAT_NEWLINE_KEYWORD).findall(sql)

        for sql_keyword in set(new_line_keywords):
            reformatted_sql = reformated_sql.replace(sql_keyword, "\n" + sql_keyword)
    ...
    def clean_sql(self) -> str:
        clean_sql = self.__decode_sql(self.reformat(self._clean_encoded_sql))
        return clean_sql
    ...
    def target_table(self) -> Optional[str]:
        ...
        if sql_type in ("CREATE TABLE", "REPLACE TABLE"):
            table_name_re = [
                re.compile(r"^CREATE .*?TABLE " + self.TBL_PATTERN, re.I),
                re.compile(r"^REPLACE .*?TABLE " + self.TBL_PATTERN, re.I),
            ]

        elif sql_type == "INSERT":
            table_name_re = [
                re.compile(r"INSERT INTO " + self.TBL_PATTERN, re.I),
                re.compile(r"INS INTO " + self.TBL_PATTERN, re.I),
            ]

        elif sql_type == "UPDATE":
            table_name_re = [
                re.compile(r"^UPDATE " + self.TBL_PATTERN, re.I),
                re.compile(r"^UPD " + self.TBL_PATTERN, re.I),
            ]

        elif sql_type == "DELETE":
            table_name_re = [
                re.compile(r"^DELETE " + self.TBL_PATTERN, re.I),
                re.compile(r"^DEL " + self.TBL_PATTERN, re.I),
            ]

        elif sql_type == "MERGE":
            table_name_re = [re.compile(r"^MERGE INTO " + self.TBL_PATTERN, re.I)]

        elif sql_type == "EXECUTE MACRO":
            table_name_re = [
                re.compile(r"^EXECUTE " + self.TBL_PATTERN, re.I),
            ...
            table_name_re = [re.compile(r"^" + sql_type + " " + self.TBL_PATTERN, re.I)]

        else:
            return None

        for tbl_regexp in table_name_re:
            target_table.extend(re.findall(tbl_regexp, self._clean_encoded_sql))
    ...
    def source_tables(self) -> List[str]:
        ...
        for tbl_regexp in table_name_re:
            source_tables.extend(re.findall(tbl_regexp, self._clean_encoded_sql))
    ...
    def sql_type(self) -> str:
        sql_type = (
            self._clean_encoded_sql.split(" ")[:5]
        ...
        if sql_type[0] == "INS":
            ...
            return "{sql_tp1} {sql_tp2}".format(
                sql_tp1=sql_type[0], sql_tp2=sql_type[1]
            )
    ...
    @property
    def sql_lang(self) -> Optional[str]:
